...
public class SkinMultipleCubes extends Group {
	...
	protected boolean reverseFront;
	
	public SkinMultipleCubes(int width, int height, int depth, float startX, float startY, double length, double thick, boolean reverseFront) {
		this.width = width;
		this.height = height;
		this.depth = depth;
		this.startX = startX;
	...
	public void updateSkin(Image skin) {
		...
		// FRONT
		getChildren().add(new Face(skin, start_x + depth_interval, start_y + depth_interval, width, height, interval, reverseFront, false,
		...
		// BACK
		getChildren().add(new Face(skin, start_x + width_interval + depth_interval * 2, start_y + depth_interval, width, height, interval, false, false,
				() -> new Box(length, length, thick), (b, p) -> {
			b.setTranslateX(((width - 1) / 2.0 - p.getX()) * b.getWidth());
			b.setTranslateY(-((height - 1) / 2.0 - p.getY()) * b.getHeight());
			b.setTranslateZ(-(depth * length + thick) / 2.0);
		...
		// TOP
		getChildren().add(new Face(skin, start_x + depth_interval, start_y, width, depth, interval, true, false,
		...
		// BOTTOM
		getChildren().add(new Face(skin, start_x + width_interval + depth_interval, start_y, width, depth, interval, true, false,
				() -> new Box(length, thick, length), (b, p) -> {
			b.setTranslateX(((width - 1) / 2.0 - p.getX()) * b.getWidth());
			b.setTranslateY((height * length + thick) / 2.0);
			b.setTranslateZ(-((depth - 1) / 2.0 - p.getY()) * b.getDepth());
